...
from models import CardFaq, Card, Faq, Supplement, HistoryCard, HistoryFaq, HistorySupplement
from models.dependencies import async_session
from configs.log_config import logger


async def save_or_update_faq(cards_id, faq_id, title, question, answer, tags, date):
...
async def card_exist(card_id: int):
    async with async_session() as session:
        async with session.begin():
            query = await session.execute(select(Card).where(Card.card_id == card_id))
            card = query.scalar()
            if card is None:
                return False
    return True
...
async def save_or_update_supplement(
    card_id,
    card_name,
    card_effect,
    card_supplement,
